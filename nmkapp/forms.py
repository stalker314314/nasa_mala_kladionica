    ...
    def clean(self):
        ...
        if 'email' in cleaned_data: 
            ...
            if len(existing_mails) > 0:
                raise forms.ValidationError({'email': [u"Ovaj e-mail je već u upotrebi. Resetuj lozinku ako je ovo Vaš mail.",]})
        return cleaned_data
...
class NewGroupForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['name'] = forms.CharField(initial='', required = True, label=u'Napravi novu ekipu', max_length=64, min_length=5)
...
class MatchForm(ModelForm):
    ...
    def clean(self):
        ...
        if Shot.objects.filter(user_round__round=this_round).exists():
            raise forms.ValidationError(u"Ne možeš dodati nove mečeve u ovo kolo jer su neki ljudi već tipovali u ovom kolu")
