...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with open(os.path.join(locale_dir, 'sr', 'LC_MESSAGES', 'django.po')) as sr_file:
            sr_content = sr_file.read()
        sr_latn_content = translit(sr_content, language_code='sr', reversed=True)
        with open(os.path.join(locale_dir, 'sr_Latn', 'LC_MESSAGES', 'django.po'), 'w') as sr_latn_file:
